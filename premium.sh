...
# ===========================================
# Auto tmux session (keeps install running)
# ===========================================
auto_tmux() {
  # If tmux exists and we are NOT already inside tmux, re-run inside tmux
  if [[ -z "$TMUX" ]] && command -v tmux >/dev/null 2>&1; then
    if [[ -z "$NA_TMUX" ]]; then
      export NA_TMUX=1
      tmux new -A -s premium "bash $0"
      exit 0
    fi
  fi
}
...
# ===========================================
# Detect public IPv4 (robust)
# ===========================================
get_public_ip() {
  local myip=""
  myip=$(curl -4fsS --connect-timeout 10 --max-time 10 https://ipv4.icanhazip.com 2>/dev/null | tr -d '\r\n') || true
  [[ -z "$myip" ]] && myip=$(curl -4fsS --connect-timeout 10 --max-time 10 https://api.ipify.org 2>/dev/null | tr -d '\r\n') || true
  echo "$myip"
}
...
license_denied_not_registered() {
  clear
  echo -e "${BLUE2}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
  echo -e "            ${red}${BOLD}404 NOT FOUND AUTOSCRIPT${NC}"
  echo -e "${BLUE2}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
  echo ""
  ...
  echo ""
  echo -e "          ${red}${BOLD}t.me/Abdulsalam03${NC}"
  echo -e "${BLUE2}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
  clean_and_exit
}
...
license_check() {
  ...
  # Re-fetch IP after DNS fix
  MYIP="$(get_public_ip)"
  if [[ -z "$MYIP" ]]; then
    clear
    echo "ERROR: Unable to detect public IP (DNS/Network problem)."
    echo "Fix DNS then try again."
    clean_and_exit
  fi
...
udp_mini(){
  ...
  safe_download "${REPO}config/services/udp-mini-3.service" /etc/systemd/system/udp-mini-3.service
...
# NEW: install limit services (shadowsocks/vmess/vless/trojan/socks)
ins_limit_services(){
  clear
  ...
  safe_download "${REPO}config/services/limitvless.service"       /etc/systemd/system/limitvless.service
  safe_download "${REPO}config/services/limitvmess.service"       /etc/systemd/system/limitvmess.service
  safe_download "${REPO}config/services/socks.service"            /etc/systemd/system/socks.service

  systemctl daemon-reload
  systemctl enable limitshadowsocks limittrojan limitvless limitvmess socks 2>/dev/null || true
  systemctl restart limitshadowsocks limittrojan limitvless limitvmess socks 2>/dev/null || true

  print_success "Limit services"
}
...
ins_epro(){
  ...
  # ws.service moved to config/services
  safe_download "${REPO}config/services/ws.service" /etc/systemd/system/ws.service
